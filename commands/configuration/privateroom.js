...
module.exports = {
	...
	async runInteraction (client, interaction) {
		...
		if (interaction.options.getSubcommand() === 'delete') {
			...
			for (const room of guild.privateRooms) {
				const getRoom = interaction.guild.channels.cache.get(room)
				row.components[0].addOptions([
					{
						label: `${getRoom.name}`,
						description: `${getRoom.parent.name}`,
						value: `${room}`
					}
				])
			}
			...
			if (guild.privateRooms.length === 0 || (guild.premium && guild.activated)) {
				const salon = interaction.options.getString('name').replace('{default}', `${guild.langue === 'fr' ? '➕ Créer votre salon' : '➕ Create your channel'}`)
